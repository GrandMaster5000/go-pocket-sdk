...
func (i AddInput) validate() error {
	if i.URL == "" {
		return errors.New("required URL values is emprty")
	}
...
func (c *Client) doHTTP(ctx context.Context, endpoint string, body interface{}) (url.Values, error) {
	...
	if err != nil {
		return url.Values{}, errors.WithMessage(err, "faild to send http request")
	}
	...
	if err != nil {
		return url.Values{}, errors.WithMessage(err, "faild to read request body")
	}

	values, err := url.ParseQuery(string(respB))
	if err != nil {
		return url.Values{}, errors.WithMessage(err, "faild to parse responce body")
	}
